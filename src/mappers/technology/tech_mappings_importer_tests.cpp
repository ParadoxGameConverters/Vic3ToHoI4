...
TEST(MappersTechnologyTechmappingsimporterTests, TechMappingsAreImported)
{
   ...
   EXPECT_THAT(tech_mappings,
       testing::ElementsAre(
           mappers::TechMapping{ std::set<std::string>{"source_tech"}, std::nullopt, std::set<std::string>{"dest_tech_one", "dest_tech_two"}},
           mappers::TechMapping{ std::set<std::string>{"source_tech_two", "source_tech_three"}, std::optional{R"(not = { has_dlc = "Test DLC" })"}, std::set<std::string>{"dest_tech_three"}}
	)
   );
}