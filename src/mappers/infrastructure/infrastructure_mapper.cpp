...
InfrastructureMapper::InfrastructureMapper(const std::map<int, vic3::State>& states)
{
   ...
   const float total_vic_infra =
       std::ranges::fold_left(states, 0.0F, [](float total, std::map<int, vic3::State>::value_type state) {
          return total + state.second.GetInfrastructure();
       });
...
int InfrastructureMapper::Map(float vic3_infrastructure)
{
   ...
   const int result = static_cast<int>(std::round(
       std::clamp(fudge_factor_.Get() + (vic3_infrastructure * hoi_infra_per_vic_infra_.Get()), 0.0F, 4.0F)));
...
float InfrastructureMapper::FindFudgeFactor(const std::map<int, vic3::State>& states)
{
   ...
   const auto function = [states, target_hoi_infra_per_state, hoi_infra_per_vic_infra](float fudge_factor) {
      const float new_infra = std::ranges::fold_left(states,
          0.0F,
          [hoi_infra_per_vic_infra, fudge_factor](float total, std::map<int, vic3::State>::value_type state) {
             return total +
                    std::round(std::clamp(fudge_factor + (state.second.GetInfrastructure() * hoi_infra_per_vic_infra),
                        0.0F,
                        4.0F));
          });
   ...
   return FindRoot<float, float>(
       ...
       0.0,
       [](float fudge) {
          return -fudge;  // good initial of fudge factor = -(difference%)
       },
       5);
}
