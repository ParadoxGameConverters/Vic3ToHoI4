...
const GraphicsBlock& GetGraphicsBlock()
{
   static const GraphicsBlock kBlock{
       .portrait_paths =
           {
...
TEST(MappersCultureCultureGraphicsMapperImporterTests, MappingsCanBeImported)  // NOLINT(cert-err58-cpp)
{
   const CultureGraphicsMapper ideology_mapper =
       ImportCultureGraphicsMapper("test_files/mappers/culture/culture_graphics.txt");

   EXPECT_EQ(ideology_mapper.MatchCultureToGraphics(vic3::CultureDefinition({.name = "swedish"})), GetGraphicsBlock());
   EXPECT_EQ(ideology_mapper.MatchCultureToGraphics(vic3::CultureDefinition({.heritage = {"heritage_nordic"}})),
       GetGraphicsBlock());
   EXPECT_EQ(ideology_mapper.MatchCultureToGraphics(vic3::CultureDefinition({.traits = {"european_heritage"}})),
       GetGraphicsBlock());
   EXPECT_EQ(ideology_mapper.MatchCultureToGraphics(vic3::CultureDefinition({.ethnicities = {"caucasian"}})),
       GetGraphicsBlock());
}


TEST(MappersCultureCultureGraphicsMapperImporterTests, NumberOfMappingsIsLogged)  // NOLINT(cert-err58-cpp)
{
