...
TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, IsValidReturnsTrueIfAnyScopeStateTrueForChildren)
{
   ...
   const AnyScopeStateTrigger any_scope_state_trigger(std::move(children));
   ...
   EXPECT_TRUE(any_scope_state_trigger.IsValid(context, world));
}


// NOLINTNEXTLINE(cert-err58-cpp)
TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, IsValidReturnsFalseIfNoOwnedStateTrueForChildren)
{
   std::unique_ptr<Trigger> one_trigger = std::make_unique<IsCapitalTrigger>(true);
   ...
   const Context context{.root = this_scope, .this_scope = this_scope, .prev = this_scope, .from = this_scope};
   EXPECT_FALSE(any_scope_state_trigger.IsValid(context, world));
}


TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, IsValidReturnsFalseIfNoOwnedStates)  // NOLINT(cert-err58-cpp)
{
...
// NOLINTNEXTLINE(cert-err58-cpp)
TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, FindAllValidReturnsStatesWithTrueCondition)
{
   ...
   const AnyScopeStateTrigger any_scope_state_trigger(std::move(any_children));

   const Country country(CountryOptions{.tag = "ONE", .owned_states = {1, 2}});
   State state_one(1, StateOptions{.owner = "ONE", .is_capital = true});
   State state_two(2, StateOptions{.owner = "ONE", .is_capital = false});
   State state_three(3, StateOptions{.owner = "ONE", .is_capital = false});
   const hoi4::World world({
       .countries =
           {
               {"ONE", country},
   ...
   EXPECT_THAT(any_scope_state_trigger.FindAllValid(context, world), testing::ElementsAre(expected));
}


TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, EquivalentTriggersAreEqual)  // NOLINT(cert-err58-cpp)
{
   ...
   const AnyScopeStateTrigger and_trigger_one(std::move(children_one));
   ...
   const AnyScopeStateTrigger and_trigger_two(std::move(children_two));
   ...
   const AnyScopeStateTrigger and_trigger_three(std::move(children_three));

   const AnyScopeStateTrigger and_trigger_four({});
...
TEST(Hoi4worldRolesTriggersAnyScopeStateTriggerTests, CopyReturnsACopy)  // NOLINT(cert-err58-cpp)
{
   std::unique_ptr<Trigger> true_trigger_one = std::make_unique<AlwaysTrigger>(true);
   std::unique_ptr<Trigger> false_trigger_one = std::make_unique<AlwaysTrigger>(false);
   std::vector<std::unique_ptr<Trigger>> children_one;
