...
TEST(Hoi4worldRolesTriggersNandTriggerTests, IsValidReturnsFalseIfAllChildrenTrue)  // NOLINT(cert-err58-cpp)
{
   ...
   EXPECT_FALSE(nand_trigger.IsValid(context, world));
}


TEST(Hoi4worldRolesTriggersNandTriggerTests, IsValidReturnsTrueIfAnyChildrenFalse)  // NOLINT(cert-err58-cpp)
{
   std::unique_ptr<Trigger> trigger_one = std::make_unique<AlwaysTrigger>(true);
   ...
   const hoi4::World world({});
   const Context context{.root = scope, .this_scope = scope, .prev = scope, .from = scope};
   EXPECT_TRUE(nand_trigger.IsValid(context, world));
}


TEST(Hoi4worldRolesTriggersNandTriggerTests, FindAllValidReturnsEmptyVector)  // NOLINT(cert-err58-cpp)
{
   std::unique_ptr<Trigger> trigger_one = std::make_unique<AlwaysTrigger>(true);
   std::unique_ptr<Trigger> trigger_two = std::make_unique<AlwaysTrigger>(false);
   ...
   EXPECT_TRUE(nand_trigger.FindAllValid(context, world).empty());
}


TEST(Hoi4worldRolesTriggersNandTriggerTests, EquivalentTriggersAreEqual)  // NOLINT(cert-err58-cpp)
{
   ...
   const NandTrigger nand_trigger_one(std::move(children_one));
   ...
   const NandTrigger nand_trigger_two(std::move(children_two));
   ...
   const NandTrigger nand_trigger_three(std::move(children_three));

   const NandTrigger nand_trigger_four({});

   EXPECT_EQ(nand_trigger_one, nand_trigger_two);
   EXPECT_NE(nand_trigger_one, nand_trigger_three);
   EXPECT_NE(nand_trigger_two, nand_trigger_three);
   EXPECT_NE(nand_trigger_one, nand_trigger_four);
   EXPECT_NE(nand_trigger_two, nand_trigger_four);
   EXPECT_NE(nand_trigger_three, nand_trigger_four);
}


TEST(Hoi4worldRolesTriggersNandTriggerTests, CopyReturnsACopy)  // NOLINT(cert-err58-cpp)
{
   std::unique_ptr<Trigger> true_trigger_one = std::make_unique<AlwaysTrigger>(true);
   std::unique_ptr<Trigger> false_trigger_one = std::make_unique<AlwaysTrigger>(false);
   std::vector<std::unique_ptr<Trigger>> children_one;
   ...
   const auto and_copy = nand_trigger_one.Copy();
   EXPECT_NE(and_copy.get(), &nand_trigger_one);
   EXPECT_EQ(*and_copy.get(), nand_trigger_one);
   ...
   children_two.push_back(std::move(true_trigger_two));
   const NandTrigger nand_trigger_two(std::move(children_two));
   EXPECT_NE(*and_copy.get(), nand_trigger_two);
}
