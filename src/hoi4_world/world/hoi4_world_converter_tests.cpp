...
TEST(Hoi4worldWorldHoi4worldconverter, CapitalsGetExtraVictoryPointValue)
{
    ...
    const hoi4::World world = hoi4::ConvertWorld(commonItems::ModFilesystem("test_files/hoi4_world/CapitalsGetExtraVictoryPointValue", {}),
        source_world,
        country_mapper,
        province_mapper,
        false);
    ...
    for (int i = 11; i < 41; ++i)
    {
        EXPECT_EQ(world.GetStates().states.at(i -1 ),
            hoi4::State(i, { .owner = fmt::format("0x0000{:0>2}", i), .provinces = {i}, .category = "rural", .victory_points = {{i, 20}} }));
    }
    ...
    {
        EXPECT_EQ(world.GetStates().states.at(i - 1),
            hoi4::State(i, { .owner = fmt::format("0x0000{:0>2}", i), .provinces = {i}, .category = "rural", .victory_points = {{i, 15}} }));
    }
    ...
    {
        EXPECT_EQ(world.GetStates().states.at(i - 1),
            hoi4::State(i, { .owner = fmt::format("0x0000{:0>2}", i), .provinces = {i}, .category = "rural", .victory_points = {{i, 10}} }));
    }
}
namespace hoi4 {
