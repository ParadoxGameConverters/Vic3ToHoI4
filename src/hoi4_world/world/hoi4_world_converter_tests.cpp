...
TEST(Hoi4worldWorldHoi4worldconverter, EmptyWorldIsEmpty)
{
   ...
   const World world = ConvertWorld(commonItems::ModFilesystem("test_files/hoi4_world", {}),
       source_world,
       mappers::WorldMapperBuilder::NullMapper().Build(),
       false);
...
TEST(Hoi4worldWorldHoi4worldconverter, CountriesAreConverted)
{
   ...
   const auto world_mapper =
       mappers::WorldMapperBuilder::NullMapper().AddCountries({{1, "TAG"}}).AddCountries({{3, "TWO"}}).Build();
...
TEST(Hoi4worldWorldHoi4worldconverter, StatesAreConverted)
{
   ...
   const auto world_mapper = mappers::WorldMapperBuilder::NullMapper()
                                 .AddProvinces({
                                     {"0x000001", {10}},
                                     {"0x000002", {20}},
                                     {"0x000003", {30}},
...
TEST(Hoi4worldWorldHoi4worldconverter, CapitalsGetExtraVictoryPointValue)
{
   ...
   mappers::WorldMapperBuilder world_mapper = mappers::WorldMapperBuilder::NullMapper();
   for (int i = 1; i <= 80; ++i)
   {
      world_mapper.AddCountries({{i, fmt::format("X{:0>2}", i)}});
      countries.emplace(i,
          vic3::Country({
              .number = i,
      ...
      world_mapper.AddProvinces({{fmt::format("0x0000{:0>2}", i), i}});
   }
...
TEST(Hoi4worldWorldHoi4worldconverter, StrategicRegionsAreCreated)
{
   ...
   mappers::WorldMapper world_mapper = mappers::WorldMapperBuilder::NullMapper()
                                           .AddProvinces({
                                               {"0x000001", {10}},
                                               {"0x000002", {20}},
                                               {"0x000003", {30}},
...
TEST(Hoi4worldWorldHoi4worldconverter, CharactersAreConverted)
{
   ...
   mappers::WorldMapper world_mapper =
