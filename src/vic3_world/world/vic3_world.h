...
class WorldBuilder
{
  public:
   ...
   /// <summary>
   /// Add test states. Each state has a separate owner (state num == state owner, starting at 1) and the provinces in the list.
   /// equivalent to {auto_increasing_int, {.owner = auto_increasing_int, .regions = {regionList}}
   /// </summary>
   WorldBuilder& AddTestStates(const std::vector<std::set<int>>& provinceListLists);
