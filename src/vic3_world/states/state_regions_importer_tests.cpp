...
TEST(Vic3worldStatesStateRegionsImporterTests, BadSpecialProvincesAreLogged)
{
    ...
    EXPECT_THAT(log.str(), testing::HasSubstr("[WARNING] Significant province x9686A5 (city) does not correspond to a province in state region STATE_SVEALAND."));
    EXPECT_THAT(log.str(), testing::HasSubstr("[WARNING] Significant province x93C3BC (port) does not correspond to a province in state region STATE_SVEALAND."));
    EXPECT_THAT(log.str(), testing::HasSubstr("[WARNING] Significant province xF48646 (farm) does not correspond to a province in state region STATE_SVEALAND."));
    EXPECT_THAT(log.str(), testing::HasSubstr("[WARNING] Significant province x6F40EC (mine) does not correspond to a province in state region STATE_SVEALAND."));
    EXPECT_THAT(log.str(), testing::HasSubstr("[WARNING] Significant province x4C9918 (wood) does not correspond to a province in state region STATE_SVEALAND."));
}
